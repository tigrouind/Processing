	...
	class Program
	{
		public static void Main()
		{
			...
			var result = parser.Rows
							.Select(x => new Tuple<DateTime, float[]>(round(DateTime.Parse(x[0])), [.. x.Skip(1).Select(y => float.Parse(y))]))
							.GroupBy(x => x.Item1, x => x.Item2)
							.Select(x => (new[] { (float)x.Key.TimeOfDay.TotalMinutes }).Concat(Parser.Transpose([.. x]).Select(y => y.Max())).ToArray())
							.ToArray();
